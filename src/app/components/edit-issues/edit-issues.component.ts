...
export class EditIssuesComponent implements OnInit, AfterViewInit {
  ...
  saveSelectedIssues() {
    ...
    if (this.local_data?.length > 0) {
      checked = this.local_data.filter((x) => x.checked).length;
      //This is if we want to set the closure Date automaticaly if the user did not set it
      this.local_data.map(async (i) => {
        if (i.checked) {
          if (
            i.status === Status.Closed.toString() &&
            i.closure_date?.length < 1
          ) {
            i.closure_date = getCurrentDate();
          }
            ...
            .then(() => {
              if (count === checked) {
                this.commonService.showMessage(
                  `Total Successfully Updated Issues: (${count}) of (${checked})  `,
                  'success'
                );
              }
            });
